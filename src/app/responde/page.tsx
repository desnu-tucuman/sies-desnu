...
export default async function SiesRespondsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const initialResponse = initialQuery ? await answerSiesRespondsQuery({ text: initialQuery, context }) : undefined;
  return <main className="contentWidth pageTop respondsPage">
    <header className="respondsPageHeading"><div><p className="eyebrow">Consulta conversacional</p><h1>SIES Responde</h1><p>Consulta en lenguaje cotidiano información sobre instituciones, carreras, autoridades y territorio.</p></div><div className="respondsPhaseBadge">Respuestas basadas en datos del SIES</div></header>
    ...
    <SiesRespondsConversation initialQuery={initialQuery} initialResponse={initialResponse} context={context} />
