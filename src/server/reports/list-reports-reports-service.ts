...
import { createGenericListPdf, createHierarchicalOffersPdf, logPdfStage } from "./pdf-export-service";
...
export async function createListPdfReport(query: ListReportQuery): Promise<GeneratedReport> {
  logPdfStage("listados", "inicio");
  const report = await getListReport(query);
  logPdfStage("listados", "carga de datos", { records: report.rows.length });
