...
const ROBOTO_REGULAR = assetPath("node_modules", "@fontsource", "roboto", "files", "roboto-latin-400-normal.woff");
const ROBOTO_BOLD = assetPath("node_modules", "@fontsource", "roboto", "files", "roboto-latin-700-normal.woff");

function configureFonts(doc: PDFKit.PDFDocument): void {
  doc.registerFont("Roboto", ROBOTO_REGULAR);
  doc.registerFont("Roboto-Bold", ROBOTO_BOLD);
  doc.font("Roboto");
}

function collectPdf(render: (doc: PDFKit.PDFDocument) => void, options: PDFKit.PDFDocumentOptions): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ ...options, font: ROBOTO_REGULAR, bufferPages: true, info: { Title: "SIES", Author: "Dirección de Educación Superior No Universitaria" } });
    const chunks: Buffer[] = [];
...
function brandHeader(doc: PDFKit.PDFDocument, compact = false): number {
  ...
  doc.font("Roboto-Bold").fillColor(DARK_BLUE).fontSize(compact ? 12 : 15).text("SIES", margin, 24);
  doc.font("Roboto").fillColor(MUTED).fontSize(compact ? 7 : 8.5)
...
function addPageFooters(doc: PDFKit.PDFDocument): void {
  ...
  for (let index = range.start; index < range.start + range.count; index += 1) {
    ...
    doc.font("Roboto").fontSize(6.4).fillColor(MUTED).text(SOURCE, doc.page.margins.left, y, { width: contentWidth - pageNumberWidth - footerGap, height: 10, lineBreak: false });
...
function listPageHeader(doc: PDFKit.PDFDocument, first: boolean, metadata: { year: string; filters: string[]; count: number }): number {
  ...
  doc.font("Roboto-Bold").fillColor(DARK_BLUE).fontSize(first ? 17 : 11).text("Listado de instituciones y sedes", doc.page.margins.left, y);
  y += first ? 27 : 20;
  if (first) {
    const filters = metadata.filters.length ? metadata.filters.join(" · ") : "Sin filtros";
    doc.font("Roboto").fillColor(MUTED).fontSize(8)
...
function offerTableHeader(doc: PDFKit.PDFDocument, y: number): number {
  let x = doc.page.margins.left;
  doc.font("Roboto-Bold").fontSize(6.4).fillColor("#FFFFFF");
...
function offerPageHeader(doc: PDFKit.PDFDocument, first: boolean, metadata: { year: string; filters: string[]; count: number }): number {
  let y = brandHeader(doc, !first);
  doc.font("Roboto-Bold").fillColor(DARK_BLUE).fontSize(first ? 17 : 11).text("Listado de oferta académica", doc.page.margins.left, y);
  y += first ? 27 : 20;
  if (first) {
    const filters = metadata.filters.length ? metadata.filters.join(" · ") : "Sin filtros";
    doc.font("Roboto").fillColor(MUTED).fontSize(8)
...
function authorityPageHeader(doc: PDFKit.PDFDocument, first: boolean, metadata: { filters: string[]; count: number }): number {
  let y = brandHeader(doc, !first);
  doc.font("Roboto-Bold").fillColor(DARK_BLUE).fontSize(first ? 17 : 11).text("Directorio de autoridades institucionales", doc.page.margins.left, y);
  y += first ? 27 : 20;
  if (first) {
    const filters = metadata.filters.length ? metadata.filters.join(" · ") : "Sin filtros";
    doc.font("Roboto").fillColor(MUTED).fontSize(8)
...
function genericListHeader(doc: PDFKit.PDFDocument, first: boolean, metadata: { title: string; year: string; filters: string[]; count: number }, columns: Array<{ label: string; width: number }>): number {
  let y = brandHeader(doc, !first);
  doc.font("Roboto-Bold").fillColor(DARK_BLUE).fontSize(first ? 16 : 10.5).text(metadata.title, doc.page.margins.left, y);
  y += first ? 26 : 19;
  if (first) {
    const year = metadata.year ? `   |   Año de referencia: ${metadata.year}` : "";
    doc.font("Roboto").fillColor(MUTED).fontSize(8).text(`Fecha de generación: ${reportDate()}${year}   |   Registros: ${metadata.count}`, doc.page.margins.left, y)
  ...
  let x = doc.page.margins.left; doc.font("Roboto-Bold").fontSize(6.5).fillColor("#FFFFFF");
...
export function createHierarchicalOffersPdf(blocks: HierarchicalOfferBlock[], metadata: { title: string; year: string; filters: string[]; count: number }): Promise<Buffer> {
  return collectPdf((doc) => {
    ...
    for (const block of blocks) {
      ...
      if (block.management !== currentManagement) { doc.rect(doc.page.margins.left, y, doc.page.width - 68, 24).fill(DARK_BLUE); doc.font("Roboto-Bold").fillColor("#FFFFFF").fontSize(10).text(`GESTIÓN ${block.management.toUpperCase()}`, doc.page.margins.left + 8, y + 7); y += 31; currentManagement = block.management; }
      doc.rect(doc.page.margins.left, y, doc.page.width - 68, blockHeight - 8).fill(LIGHT).strokeColor(LINE).lineWidth(.5).stroke();
      doc.font("Roboto-Bold").fillColor(BLUE).fontSize(10).text(block.institution, doc.page.margins.left + 10, y + 9, { width: doc.page.width - 88 });
      doc.font("Roboto").fillColor(MUTED).fontSize(7.5).text(`Localidad: ${block.locality || NO_DATA}   |   Departamento: ${block.department || NO_DATA}   |   Tipo de sede: ${block.siteType || NO_DATA}`, doc.page.margins.left + 10, y + 25);
      doc.font("Roboto-Bold").fillColor(TURQUOISE).fontSize(7.5).text("Oferta académica:", doc.page.margins.left + 10, y + 39); let offerY = y + 51;
      doc.font("Roboto").fillColor("#18313F").fontSize(8); block.offers.forEach((offer) => { doc.text(`• ${offer}`, doc.page.margins.left + 18, offerY, { width: doc.page.width - 105, height: 11, ellipsis: true }); offerY += 12; });
      y += blockHeight;
    }
  }, { size: "A4", layout: "landscape", margins: { top: 18, bottom: 42, left: 34, right: 34 } });
}
...
export function createInstitutionProfilePdf(institution: InstitutionView): Promise<Buffer> {
  return collectPdf((doc) => {
    ...
    const heading = (number: string, title: string) => {
      ...
      doc.font("Roboto-Bold").fontSize(14).fillColor("#9ED9DC").text(number, 48, y + 9, { width: 30 });
    ...
    const fieldRows = (fields: Array<[string, string | undefined]>) => {
      for (let index = 0; index < fields.length; index += 2) {
        ...
        pair.forEach(([label, raw], pairIndex) => {
          ...
          doc.font("Roboto-Bold").fontSize(6.7).fillColor(MUTED).text(label.toUpperCase(), x + 7, y + 5, { width: width - 14 });
          doc.font("Roboto").fontSize(8.5).fillColor("#18313F").text(value(raw), x + 7, y + 16, { width: width - 14, ellipsis: true });
        }); y += 34;
      }
      y += 10;
    };

    doc.font("Roboto-Bold").fontSize(17).fillColor(DARK_BLUE).text("Ficha institucional", doc.page.margins.left, y);
    y += 25; doc.font("Roboto-Bold").fontSize(15).fillColor(BLUE).text(institution.name, doc.page.margins.left, y, { width: doc.page.width - 80 });
    ...
    if (!institution.authorities.length) {
      doc.font("Roboto").fontSize(9).fillColor(MUTED).text(NO_DATA, 48, y); y += 26;
      ...
      doc.font("Roboto-Bold").fontSize(9.5).fillColor(DARK_BLUE).text(value(authority.name), 52, y + 8, { width: 260 });
      doc.font("Roboto-Bold").fontSize(7.5).fillColor(TURQUOISE).text(value(authority.role), 52, y + 24, { width: 260 });
      doc.font("Roboto").fontSize(7.5).fillColor(MUTED).text(`Teléfono: ${value(authority.phone)}\nCorreo: ${value(authority.email)}`, 325, y + 9, { width: 220 });
      y += 61;
    });
    y += 6;
    ...
    for (const [title, items] of lists) {
      ensure(32); doc.font("Roboto-Bold").fontSize(10).fillColor(DARK_BLUE).text(title, 40, y); y += 17;
      if (!items?.length) { doc.font("Roboto").fontSize(8.5).fillColor(MUTED).text(NO_DATA, 48, y); y += 20; continue; }
      for (const item of items) {
        const height = Math.max(18, doc.heightOfString(item, { width: 485 }) + 7); ensure(height);
        doc.circle(48, y + 5, 2).fill(TURQUOISE); doc.font("Roboto").fontSize(8.5).fillColor("#18313F").text(item, 58, y, { width: 485 }); y += height;
      }
      y += 4;
    }
